...
export default function ContactClient() {
  ...
  useEffect(() => {
    ...
    fetch("/api/settings", { cache: "no-store", signal: controller.signal })
      ...
      .then(async (data) => {
        ...
        if (data && !data.error) {
          setSettings(data);
          await waitForConfiguredFonts(data);
        }
  ...
  const customFontStyle = (font?: string) =>
    font && font !== "Inter" ? { fontFamily: `'${font}', sans-serif` } : {};
  ...
  if (!settingsReady) {
    return (
      <div className="container mx-auto px-4 py-24 min-h-screen max-w-7xl animate-pulse">
        <div className="mb-16">
          <div className="h-12 w-2/3 max-w-xl rounded-lg bg-gray-200 dark:bg-gray-800 mb-6" />
          <div className="h-5 w-full max-w-2xl rounded bg-gray-200 dark:bg-gray-800" />
          <div className="h-5 w-4/5 max-w-xl rounded bg-gray-200 dark:bg-gray-800 mt-3" />
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20">
          ...
          <div className="space-y-5">
            ...
            <div className="h-12 w-full rounded-xl bg-gray-200 dark:bg-gray-800" />
            <div className="h-12 w-full rounded-xl bg-gray-200 dark:bg-gray-800" />
            <div className="h-36 w-full rounded-2xl bg-gray-200 dark:bg-gray-800" />
          </div>
        </div>
      </div>
    );
    ...
    <div className="container mx-auto px-4 py-24 min-h-screen flex flex-col justify-center relative max-w-7xl">
      ...
      <AnimatedSection direction="up">
        <div className="mb-16 text-center md:text-left">
          ...
          >
            {settings?.contactSubheader ||
              "I'm currently open to new opportunities, freelance projects, and creative collaborations. Reach out and let's chat!"}
          </p>
        </div>
      </AnimatedSection>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-start">
        ...
        <div className="space-y-8">
          ...
          <AnimatedSection direction="up" delay={0.1}>
            <div className="flex flex-col sm:flex-row gap-4 mb-8">
              <div
                style={customFontStyle(settings?.contactLocationFont)}
                className="flex items-center gap-2.5 px-4 py-2.5 bg-gray-100 dark:bg-gray-900 rounded-xl text-sm font-medium text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-800"
              >
                <MapPin className="w-4 h-4 text-primary" />
                Siliguri, India • IST (UTC+5:30)
              </div>
              <div
                style={customFontStyle(settings?.contactAvailabilityFont)}
                className="flex items-center gap-2.5 px-4 py-2.5 bg-gray-100 dark:bg-gray-900 rounded-xl text-sm font-medium text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-800"
              >
        ...
        <AnimatedSection direction="up" delay={0.2}>
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-3xl p-8 shadow-xl relative overflow-hidden">
            {/* Success Overlay */}
            <div
              style={customFontStyle(settings?.contactFormStatusFont)}
              className={`absolute inset-0 bg-white/95 dark:bg-gray-900/95 backdrop-blur-sm z-10 flex flex-col items-center justify-center transition-all duration-500 ${submitStatus === "success" ? "opacity-100 visible" : "opacity-0 invisible"}`}
            >
            ...
            <h2
              style={headingFontStyle(settings?.contactFormHeadingFont)}
            ...
            {submitStatus === "error" && (
              <div
                style={customFontStyle(settings?.contactFormStatusFont)}
                className="mb-6 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl flex items-start gap-3 text-red-600 dark:text-red-400"
              >
            ...
            <form onSubmit={handleFormSubmit} className="space-y-5">
              <div>
                <label
                  style={customFontStyle(settings?.contactFormLabelFont)}
                  htmlFor="name"
