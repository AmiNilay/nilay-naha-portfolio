...
export default function BlogClient() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (settingsRes && settingsRes.ok) {
        ...
        await waitForConfiguredFonts(settingsData);
      }
      ...
      if (err instanceof DOMException && err.name === "AbortError") {
        ...
        setError(
          "Failed to load articles. The server might be busy or experiencing issues.",
        );
      }
    } finally {
      setLoading(false);
  ...
  const filteredPosts = posts.filter((post) => {
    ...
    const matchesFilter =
      ...
      pTags.some((t) => t.trim() === activeFilter);
  ...
  return (
    <div className="container mx-auto px-4 py-24 min-h-screen max-w-7xl">
      <AnimatedSection direction="up">
        <div className="max-w-4xl mx-auto mb-12 text-center">
          <h1
            ...
            style={{
              fontFamily:
                settings?.blogHeaderFont && settings.blogHeaderFont !== "Inter"
                  ? `'${settings.blogHeaderFont}', sans-serif`
                  : "inherit",
              fontWeight:
                settings?.blogHeaderFont && settings.blogHeaderFont !== "Inter"
                  ? "normal"
          ...
          <p
            ...
            style={{
              fontFamily:
                settings?.blogSubheaderFont &&
                settings.blogSubheaderFont !== "Inter"
                  ? `'${settings.blogSubheaderFont}', sans-serif`
                  : "inherit",
            }}
          >
            {settings?.blogSubheader ||
      ...
      {(loading || !settingsReady) && !error && (
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div
              key={i}
              className="flex flex-col h-[420px] bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-3xl overflow-hidden shadow-sm animate-pulse"
            >
      ...
      {/* EMPTY STATE */}
      {!loading && settingsReady && !error && filteredPosts.length === 0 && (
                    ...
                    {displayTags.length > 0 && (
                      <div className="flex flex-wrap gap-2 mb-4">
                        {displayTags.map((tag, i) => (
                          <span
                            key={i}
                            className="px-2.5 py-1 bg-primary/10 text-primary text-[10px] font-bold uppercase tracking-wider rounded-md"
                          >
                            {tag.trim()}
                          </span>
                        ))}
