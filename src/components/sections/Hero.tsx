...
const getTechLogo = (tech: string) => {
  ...
  if (t.includes("fastapi"))
    return "https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/fastapi/fastapi-original.svg";
  ...
  if (t.includes("linux") || t.includes("ubuntu"))
...
export default function Hero() {
  ...
  const imageKey = data.profilePic
    ? `${data.profilePic}?v=${Date.now()}`
    : null;
  const easeOut = [0.25, 0.4, 0.25, 1] as const;
  const techArray = data.techStack
    ? data.techStack
        .split(",")
        .map((t) => t.trim())
        .filter(Boolean)
      ...
      <div className="w-full max-w-[1400px] mx-auto px-3 sm:px-10 lg:px-16 flex flex-col md:flex-row items-center justify-start md:justify-between gap-3 sm:gap-6 md:gap-12 lg:gap-16 h-full pb-16 md:pb-12 pt-16 md:pt-20">
        ...
        >
            ...
            >
              {data.socialGithub && (
                <Link
                  ...
                  target="_blank"
                  className="p-3 md:p-3.5 rounded-full border border-gray-200 dark:border-gray-700 bg-white dark:bg-transparent hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors text-gray-600 dark:text-gray-400 hover:text-black dark:hover:text-white"
                >
              ...
              {data.socialLinkedin && (
                <Link
                  ...
                  target="_blank"
                  className="p-3 md:p-3.5 rounded-full border border-gray-200 dark:border-gray-700 bg-white dark:bg-transparent hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors text-gray-600 dark:text-gray-400 hover:text-blue-600"
                >
            ...
            >
              {data.stat1Value && (
                <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-xl md:rounded-2xl p-3 md:p-5 flex flex-col items-center justify-center text-center shadow-sm">
                  <h4
                    className="text-xl sm:text-3xl md:text-4xl font-extrabold text-[#111827] dark:text-white"
                    style={customFontStyle(settings?.homeStatsValueFont)}
                  >
                    {data.stat1Value}
                  </h4>
                  <p
                    className="text-[8px] sm:text-[10px] md:text-xs text-gray-500 font-bold uppercase tracking-widest mt-1 md:mt-1.5"
                    style={customFontStyle(settings?.homeStatsLabelFont)}
                  >
                    {data.stat1Label}
                  </p>
                </div>
          ...
          {data.portfolioLastUpdated && (
            ...
            >
              ...
              <span
                className="text-[10px] md:text-xs font-semibold text-green-700 dark:text-green-500"
                style={customFontStyle(settings?.homeLastUpdatedFont)}
              >
                {data.portfolioLastUpdated}
              </span>
            </motion.div>
        ...
        >
          <motion.div
            ...
            className="absolute inset-0 -z-10 blur-[60px] md:blur-[100px] opacity-60 dark:opacity-40"
          >
          ...
          >
            <Image
              key={imageKey}
              src={
                data.profilePic || data.gDriveProfilePic || "/placeholder.png"
              }
              alt="Profile Picture"
              fill
